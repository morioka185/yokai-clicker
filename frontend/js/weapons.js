...
const Weapons = {
    ...
    clickSpear(e) {
        ...
        if (timingEl) {
            ...
            timingEl.style.color = diff <= 8 ? 'var(--accent-gold)' :
                                   diff <= 18 ? 'var(--accent-green)' :
                                   diff <= 30 ? 'var(--text-secondary)' : 'var(--accent-red)';
        }
            ...
            <div class="weapon-ui-inner fist-theme">
                ...
                <div class="weapon-main-display">
                    ...
                    <div class="fist-tiers">
                        <span class="fist-tier">3+:疾風</span>
                        <span class="fist-tier">6+:練気</span>
                        <span class="fist-tier">10+:無双</span>
                    </div>
    ...
    clickFist(e) {
        ...
        if (cps >= 10) {
            multiplier = 1.5; rank = '無双'; rankColor = '#ff1744';
            isMusou = true;
        } else if (cps >= 6) {
            multiplier = 1.0; rank = '練気'; rankColor = '#ff9800';
            isRenki = true;
        } else if (cps >= 3) {
            multiplier = 0.7; rank = '疾風'; rankColor = '#4caf50';
        } else {
            multiplier = 0.3; rank = '通常'; rankColor = '#9e9e9e';
        }
        ...
        if (isMusou) {
            const allAlive = Combat.getAliveEnemies();
            Combat.performClick(multiplier, allAlive, 0.7);
        } else if (isRenki) {
            ...
            Combat.performClick(multiplier, targets, 0.8);
        } else {
            Combat.performClick(multiplier, [target]);
        }
    ...
    updateFist(dt) {
        ...
        const rankEl = document.getElementById('fist-rank');
        if (rankEl) {
            if (cps >= 10) { rankEl.textContent = '無双'; rankEl.style.background = '#ff1744'; }
            else if (cps >= 6) { rankEl.textContent = '練気'; rankEl.style.background = '#ff9800'; }
            else if (cps >= 3) { rankEl.textContent = '疾風'; rankEl.style.background = '#4caf50'; }
        }
    },
    ...
    clickStaffNode(nodeIndex) {
        ...
        if (s.pattern[s.currentIndex] === nodeIndex) {
            ...
            if (s.currentIndex >= s.pattern.length) {
                // パターン完成！→ 全敵AoE
                s.completed = true;
                const multiplier = 2.5 + s.pattern.length * 0.8;
                const allAlive = Combat.getAliveEnemies();
    ...
    _effectSpear(coords) {
        ...
        let timing;
        if (diff <= 8) timing = 'perfect';
        else if (diff <= 18) timing = 'great';
        else if (diff <= 30) timing = 'good';
        else timing = 'miss';
    ...
    _effectFist(coords) {
        ...
        let rank;
        if (cps >= 10) rank = 'rank-musou';
        else if (cps >= 6) rank = 'rank-renki';
        else if (cps >= 3) rank = 'rank-shippuu';
